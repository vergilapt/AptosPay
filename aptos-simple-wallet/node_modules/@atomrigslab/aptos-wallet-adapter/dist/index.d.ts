import { Account } from '@aptos-labs/ts-sdk';
import { AptosConnectMethod } from '@aptos-labs/wallet-standard';
import { AptosDisconnectMethod } from '@aptos-labs/wallet-standard';
import { AptosFeatures } from '@aptos-labs/wallet-standard';
import { AptosGetAccountMethod } from '@aptos-labs/wallet-standard';
import { AptosGetNetworkMethod } from '@aptos-labs/wallet-standard';
import { AptosOnAccountChangeMethod } from '@aptos-labs/wallet-standard';
import { AptosOnNetworkChangeMethod } from '@aptos-labs/wallet-standard';
import { AptosSignAndSubmitTransactionMethod } from '@aptos-labs/wallet-standard';
import { AptosSignMessageOutput } from '@aptos-labs/wallet-standard';
import { AptosSignTransactionMethod } from '@aptos-labs/wallet-standard';
import { AptosWallet } from '@aptos-labs/wallet-standard';
import { AptosWalletAccount } from '@aptos-labs/wallet-standard';
import { IdentifierArray } from '@aptos-labs/wallet-standard';
import { SigningScheme } from '@aptos-labs/ts-sdk';
import { UserResponse } from '@aptos-labs/wallet-standard';

export declare class DevTWallet extends TWallet {
    readonly url: string;
    readonly name: string;
    constructor(option?: TWalletOption);
}

declare type ENV = "LOCAL" | "DEV" | "QA" | "PROD";

export declare class LocalTWallet extends TWallet {
    readonly url: string;
    readonly name: string;
    constructor(option?: TWalletOption);
}

export declare class MyWalletAccount implements AptosWalletAccount {
    address: string;
    publicKey: Uint8Array;
    chains: IdentifierArray;
    features: IdentifierArray;
    signingScheme: SigningScheme;
    label?: string;
    icon?: `data:image/svg+xml;base64,${string}` | `data:image/webp;base64,${string}` | `data:image/png;base64,${string}` | `data:image/gif;base64,${string}` | undefined;
    constructor(account: Account);
}

export declare class QaTWallet extends TWallet {
    readonly url: string;
    readonly name: string;
    constructor(option?: TWalletOption);
}

export declare class TWallet implements AptosWallet {
    private option?;
    readonly url: string;
    readonly version = "1.0.0";
    readonly name: string;
    readonly icon = "data:image/png;base64,";
    chains: readonly ["aptos:devnet", "aptos:testnet", "aptos:localnet", "aptos:mainnet"];
    accounts: MyWalletAccount[];
    provider: any;
    initializing: boolean;
    get features(): AptosFeatures;
    constructor(option?: {
        registerWallet?: boolean;
        env?: ENV;
    });
    account: AptosGetAccountMethod;
    connect: AptosConnectMethod;
    checkProvider: () => Promise<unknown>;
    network: AptosGetNetworkMethod;
    disconnect: AptosDisconnectMethod;
    signTransaction: AptosSignTransactionMethod;
    signAndSubmitTransaction: AptosSignAndSubmitTransactionMethod;
    signMessage: (input: any) => Promise<UserResponse<AptosSignMessageOutput>>;
    onAccountChange: AptosOnAccountChangeMethod;
    onNetworkChange: AptosOnNetworkChangeMethod;
}

declare type TWalletOption = {
    registerWallet?: boolean;
};

export { }
