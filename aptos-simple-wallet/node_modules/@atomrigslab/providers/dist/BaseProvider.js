...
class BaseProvider extends safe_event_emitter_1.default {
    ...
    _handleAccountsChanged(accounts, isEthAccounts = false) {
        ...
        if (!Array.isArray(accounts)) {
            this._log.error('Dekey: Received invalid accounts parameter. Please report this bug.', accounts);
            _accounts = [];
        }
        for (const account of accounts) {
            if (typeof account !== 'string') {
                this._log.error('Dekey: Received non-string account. Please report this bug.', accounts);
                _accounts = [];
                break;
            }
        }
        ...
        if (!fast_deep_equal_1.default(this._state.accounts, _accounts)) {
            ...
            if (isEthAccounts && this._state.accounts !== null) {
                this._log.error(`Dekey: 'eth_accounts' unexpectedly updated accounts. Please report this bug.`, _accounts);
            }
    ...
    _handleUnlockStateChanged({ accounts, isUnlocked, } = {}) {
        if (typeof isUnlocked !== 'boolean') {
            this._log.error('Dekey: Received invalid isUnlocked parameter. Please report this bug.');
            return;
        }
